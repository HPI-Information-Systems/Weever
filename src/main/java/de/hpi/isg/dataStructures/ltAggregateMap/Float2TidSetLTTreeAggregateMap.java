...
public class Float2TidSetLTTreeAggregateMap extends AbstractFloat2ObjectSortedMap<TIdSet>
    ...
    private Operand findSmallerTIdsImplFastAgg(final float k, boolean inclusive) {
        ...
        return new Operand(new RoaringTidSet(FastAggregation.or(results.iterator())), equalVals, inclusive);
    }
    ...
    @Override
    public boolean isCloserToMapStart(Number k) {
        float query = (float) k;
        if (this.firstEntry == null) return true;

        float diffToFirst = Math.abs(query - this.firstEntry.getFloatKey());
        float diffToLast = Math.abs(query - this.lastEntry.getFloatKey());
        return diffToFirst < diffToLast;
    }
